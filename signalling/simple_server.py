...
class WebRTCSimpleServer(object):
    ...
    def get_raddr( self, ws ):
        print( f" get_raddr ws.remote_address = {ws.remote_address}")
        print( f" get_raddr ws.request_headers.get('X-Forwarded-For') = {ws.request_headers.get('X-Forwarded-For')}")
        xforwardedfor = ws.request_headers.get('X-Forwarded-For')
        if isinstance(xforwardedfor, str):
            ...
            if isinstance( clientiplistxforwardedfor, list ):
                for ipforwarded in clientiplistxforwardedfor:
                    ...
                    if isinstance(ipforwarded, str) and len(ipforwarded)>=7:
                        return ipforwarded
    ...
    async def connection_handler(self, ws, uid):
        ...
        raddr = self.get_raddr( ws )
        print( f" connection_handler raddr = {raddr}" )
        peer_status = None
    ...
    async def hello_peer(self, ws):
        ...
        # raddr = ws.remote_address
        raddr = self.get_raddr( ws )
        print( f" hello_peer raddr = {raddr}" )
        hello = await ws.recv()
        print(hello)
    ...
    def run(self):
        ...
        async def handler(ws, path):
            ...
            print("await self.hello_peer(ws)")
        ...
        # Run the server
        print("self.loop.run_until_complete(wsd)")
